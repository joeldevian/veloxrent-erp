            ...
            {loading ? <tr><td colSpan="7" style={{ textAlign: 'center', padding: 40 }}>Cargando...</td></tr> :
              contracts.length === 0 ? <tr><td colSpan="7" style={{ textAlign: 'center', padding: 40, color: '#94A3B8' }}>No hay registros en esta vista.</td></tr> :
                contracts.map(c => {
                  ...
                  const days = calculateDays(c.start_datetime, c.end_datetime_planned);
                  const rowBg = delayed ? '#FEF2F2' : isCancelled ? '#FFF1F2' : isClosed ? '#F8FAFC' : 'transparent';
                  return (
                    <tr key={c.id} style={{ background: rowBg, opacity: isCancelled ? 0.7 : 1 }}>
                      <td>
                        <strong style={{ color: delayed ? '#B91C1C' : '#0F172A', textDecoration: isCancelled ? 'line-through' : 'none' }}>{c.clients?.full_name || '—'}</strong><br />
                        <span style={{ fontSize: 12, color: '#64748B' }}>{c.clients?.phone}</span>
                      </td>
                      <td>
                        <span style={{ fontWeight: 600, color: '#334155', textDecoration: isCancelled ? 'line-through' : 'none' }}>{c.vehicles ? `${c.vehicles.brand} ${c.vehicles.model}` : '—'}</span><br />
                      ...
                      <td>
                        ...
                        <div style={{ fontSize: 13, color: '#0F172A', marginTop: 6, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ background: '#EFF6FF', color: '#2563EB', padding: '2px 8px', borderRadius: 4, fontSize: 12, fontWeight: 800 }}>{days} {days === 1 ? 'día' : 'días'}</span>
                          {c.trip_destination && (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#64748B', fontWeight: 600, fontSize: 13 }}><MapPin size={12} color="#94A3B8" /> {c.trip_destination}</span>
                          )}
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        <strong style={{ color: isCancelled ? '#94A3B8' : '#22C55E', textDecoration: isCancelled ? 'line-through' : 'none' }}>{c.total_amount > 0 ? `S/ ${parseFloat(c.total_amount).toFixed(2)}` : 'S/ --'}</strong>
                      </td>
                      <td>
